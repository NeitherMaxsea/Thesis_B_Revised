@php
    ...
    $employerPhotoUrl = $employerUser?->profile_photo_url;
    $employerWorkspacePage = request()->routeIs('messages.*')
        ? 'Chat'
        : (request()->routeIs('employer.profile*') ? 'Settings' : 'Job posting');
...
<body class="employer-page" data-auth-user-id="{{ $employerUser?->id }}" data-account-type="{{ $employerUser?->account_type }}" data-unread-message-count="{{ $layoutUnreadMessageCount }}" data-inbox-updates-url="{{ route('messages.inbox-updates') }}" data-inbox-message-cursor="{{ max(0, (int) ($inboxMessageCursor ?? 0)) }}">
    <div class="employer-shell">
        <aside class="employer-sidebar" aria-label="Employer navigation">
            ...
            <nav>
                <a href="{{ route('employer.dashboard') }}" class="{{ request()->routeIs('employer.dashboard') ? 'is-active' : '' }}">
                ...
                <a href="{{ route('messages.index') }}" class="{{ request()->routeIs('messages.*') ? 'is-active' : '' }}">
                ...
                <a href="{{ route('employer.profile') }}" class="{{ request()->routeIs('employer.profile*') ? 'is-active' : '' }}">
        ...
        <div class="employer-content">
            <header class="employer-topbar">
                <div class="employer-topbar__page">
                    <span>Employer workspace</span>
                    <strong>{{ $employerWorkspacePage }}</strong>
                </div>
                <div class="employer-topbar__actions">
                    ...
                    <div class="dashboard-profile-menu" data-dashboard-profile-menu>
                        <button type="button" class="dashboard-profile-menu__toggle" data-dashboard-profile-toggle aria-expanded="false" aria-controls="employer-profile-options">
                            <span class="dashboard-profile-menu__avatar {{ $employerPhotoUrl ? 'has-image' : '' }}" data-employer-navbar-avatar aria-hidden="true">
                                @if ($employerPhotoUrl)
                                    <img src="{{ $employerPhotoUrl }}" alt="">
                                @else
                                    {{ $employerInitials ?: 'EH' }}
                                @endif
                            </span>
                            <span class="dashboard-profile-menu__copy"><strong data-employer-navbar-name>{{ $employerName }}</strong><small>Employer account</small></span>
                            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 10 5 5 5-5" /></svg>
                        </button>
                        <div id="employer-profile-options" class="dashboard-profile-menu__options" data-dashboard-profile-options hidden>
                            <a href="{{ route('employer.profile') }}"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.25" /><path d="M5.5 20a6.5 6.5 0 0 1 13 0" /></svg> Edit profile</a>
